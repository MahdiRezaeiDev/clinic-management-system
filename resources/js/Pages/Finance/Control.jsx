...
export default function Control({
    ...
    const apply = (e) => {
        e.preventDefault();
        router.get(route('finance.control'), form, { preserveState: true });
    };
    const balance = Number(summary.credits) - Number(summary.debits);
    return (
        <AuthenticatedLayout title="کنترل مالی و صندوق">
            ...
            <ClinicPage className="space-y-6">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    ...
                    <Card
                        title="کل پرداختی"
                        value={money(summary.debits)}
                        tone="text-red-600"
                    />
                    <Card
                        title="مانده صندوق"
                        value={money(balance)}
                        tone={balance >= 0 ? 'text-blue-600' : 'text-red-600'}
                    />
                </div>
                <Section title="تفکیک روش پرداخت">
                    <Table
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                    <Section title="بدهی به تأمین‌کنندگان">
                        <Table
                ...
                <Section title="هشدار موجودی و انقضا">
                    <Table
                        headers={['دوا', 'موجودی', 'حد سفارش', 'انقضا']}
                        rows={inventoryAlerts.map((x) => [
                            x.brand_name,
                            x.stock_quantity,
                            x.reorder_level,
                ...
                <Section title="آخرین تغییرات مالی">
                    <Table
                        headers={['مدل', 'عملیات', 'شناسه', 'زمان']}
                        rows={auditLogs.map((x) => [
                    ...
                    <div className="flex flex-wrap gap-2">
                        {transactions.links.map((l, i) => (
                            <Link
                                key={i}
                                href={l.url || '#'}
                                className={`rounded border px-3 py-1 ${l.active ? 'bg-teal-600 text-white' : 'bg-white'}`}
...
function Section({ title, children }) {
    return (
        <ClinicPanel title={title} bodyClassName="p-0">
            {children}
        </ClinicPanel>
    );
}
                ...
                <tbody>
                    {rows.length ? (
                        rows.map((r, i) => (
                            <tr
                                key={i}
                                className="border-t border-gray-100 transition hover:bg-teal-50/50"
                            >
                                {r.map((c, j) => (
                                    <td
                                        key={j}
