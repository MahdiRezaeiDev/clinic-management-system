...
const Card = ({ title, value, tone = 'text-gray-900' }) => (
    <div className="rounded-xl border bg-white p-5 shadow-sm"><p className="text-sm text-gray-500">{title}</p><p className={`mt-2 text-2xl font-bold ${tone}`}>{value}</p></div>
);
...
export default function Control({ transactions, summary, supplierDebts, patientDebts, inventoryAlerts, auditLogs, filters }) {
    ...
    return <AuthenticatedLayout title="کنترل مالی و صندوق">
        ...
        <div dir="rtl" className="mx-auto max-w-7xl space-y-6 px-4 py-8">
            <div><h1 className="text-2xl font-bold">کنترل مالی و صندوق</h1><p className="text-sm text-gray-500">دفتر واحد تراکنش‌ها، بدهی‌ها، هشدارها و تاریخچه تغییرات</p></div>
            <form onSubmit={apply} className="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-4">
                <input className="rounded-lg border-gray-300" placeholder="از تاریخ 1405/01/01" value={form.start_date} onChange={e => setForm({...form, start_date:e.target.value})}/>
                <input className="rounded-lg border-gray-300" placeholder="تا تاریخ 1405/12/29" value={form.end_date} onChange={e => setForm({...form, end_date:e.target.value})}/>
                <select className="rounded-lg border-gray-300" value={form.payment_method} onChange={e => setForm({...form, payment_method:e.target.value})}><option value="">همه روش‌ها</option><option value="cash">نقدی</option><option value="card">کارت</option><option value="bank">بانک</option><option value="check">چک</option><option value="other">سایر</option></select>
                <button className="rounded-lg bg-teal-600 px-4 py-2 text-white">اعمال فیلتر</button>
            </form>
...
function Section({title,children}) { return <section className="overflow-hidden rounded-xl border bg-white shadow-sm"><h2 className="border-b px-5 py-4 font-bold">{title}</h2>{children}</section>; }
function Table({headers,rows}) { return <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-gray-50"><tr>{headers.map(h=><th key={h} className="px-4 py-3 text-right">{h}</th>)}</tr></thead><tbody>{rows.length?rows.map((r,i)=><tr key={i} className="border-t">{r.map((c,j)=><td key={j} className="whitespace-nowrap px-4 py-3">{c}</td>)}</tr>):<tr><td colSpan={headers.length} className="p-6 text-center text-gray-400">موردی موجود نیست</td></tr>}</tbody></table></div>; }
